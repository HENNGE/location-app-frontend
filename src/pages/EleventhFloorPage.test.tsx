...
import EleventhFloorPage from './EleventhFloorPage';

describe('EleventhFloorPage', () => {
    test('renders map correctly', async () => {
        ...
        expect(
            screen.getByRole('button', {
                name: /11F Guest Meeting Rooms area button/i,
            })
        ).toBeInTheDocument();

        expect(
            screen.getByRole('button', {
                name: /11F Open Lounge area button/i,
            })
        ).toBeInTheDocument();
    });
    ...
    test('handles drawer opening correctly', async () => {
        ...
        expect(
            screen.getByRole('dialog', { name: /11F Guest Meeting Rooms/i })
        ).toBeInTheDocument();
        ...
        expect(
            screen.getByRole('dialog', { name: /11F Open Lounge/i })
        ).toBeInTheDocument();
    });
});
